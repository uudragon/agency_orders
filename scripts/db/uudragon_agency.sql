CREATE TABLE IF NOT EXISTS `AGENCY`.`T_ORDERS` (
  ...
  `CUSTMER_NAME` varchar(200) NOT NULL COMMENT '客户名称',
  `CUSTMER_PHONE` varchar(32) DEFAULT NULL COMMENT '客户手机',
  `CUSTMER_TEL` varchar(32) DEFAULT NULL COMMENT '客户电话',
  `CUSTMER_ADDR` varchar(200) DEFAULT NULL COMMENT '客户地址',
  ...
  `CREATOR` varchar(32) DEFAULT NULL COMMENT '受理人',
...
CREATE TABLE IF NOT EXISTS `AGENCY`.`T_ORDERS_DETAILS` (
  ...
  `TYPE` TINYINT(4) NOT NULL COMMENT '0：首期；1：宣传品' DEFAULT 0,
  `QTY` INT COMMENT '应发数量' DEFAULT 1,
  `AMOUNT` DECIMAL(20,2) COMMENT '金额' DEFAULT 0.00,

  PRIMARY KEY (`ID`),
  UNIQUE INDEX `ORDER_DETAILS_UNIQUE` (`ORDERS_NO` ASC,`CODE` ASC)
  )
ENGINE = InnoDB
COMMENT = '订单明细表';